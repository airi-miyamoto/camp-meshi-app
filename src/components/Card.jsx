...
function Card({ id, image, name}) {
    return (
        <Link to={`/detail/${id}`} className="group cursor-pointer text-left block drop-shadow-sm dark:drop-shadow-white aspect-[3/4] w-full h-fullrelative">
            ...
            <h3 className="pb-[16px] px-[16px] text-white flex items-center gap-[8px] text-lg font-bold absolute bottom-0 left-0 z-[2] font-sans"><PiHashBold className="shrink-0" />{name}</h3>
        </Link>
    )
}
